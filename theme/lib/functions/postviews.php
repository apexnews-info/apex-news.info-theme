...
function bzb_add_postviews_column_content($column_name)
{
  if ($column_name == 'views') {
    if (function_exists('bzb_the_views')) {
      bzb_the_views(true, '', '', true);
    }
  }
}
...
function bzb_pre_views_activation()
{
  if (false === get_option('theme_activated')) {
    ...
    $option = array(
      'count'                   => 1, 'exclude_bots'            => 0, 'display_home'            => 0, 'display_single'          => 0, 'display_page'            => 0, 'display_archive'         => 0, 'display_search'          => 0, 'display_other'           => 0, 'use_ajax'                => 1, 'template'                => __('%VIEW_COUNT% views', 'wp-postviews'), 'most_viewed_template'    => '<li><a href="%POST_URL%"  title="%POST_TITLE%">%POST_TITLE%</a><span> %VIEW_COUNT% ' . __('PV', 'wp-postviews') . '</span></li>'
    );
